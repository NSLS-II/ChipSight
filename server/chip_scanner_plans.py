...
BL_calibration = BeamlineCalibrations(
    "XF:17ID-ES:FMX{Misc-", name="BL_calibration", read_attrs=["LoMagCal", "HiMagCal"]
)
...
class ChipScanner(Device):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lo_camera_ratios = (
            BL_calibration.LoMagCal.get(),
            BL_calibration.LoMagCal.get(),
        )
        self.hi_camera_ratios = (
            BL_calibration.HiMagCal.get(),
            BL_calibration.HiMagCal.get(),
        )
    ...
    def drive_to_fiducial(self, location_name):
        ...
        if location is None:
            yield from self.drive_to_position(
                approx_locations[location_name][0], approx_locations[location_name][1]
            )
        else:
    ...
    def find_fiducials(self):
        ...
        if not (
            self.check_camera_settings(self.lo_camera)
            and self.check_camera_settings(self.hi_camera)
        ):
        ...
        for i in range(3):
            location = f"F{i}"
            yield from bps.mv(
                self.x, locations[location][0], self.y, locations[location][1]
            )
            success = yield from self.find_center(location, center_high=True)
            if success:
                posns = np.array(
                    [
                        getattr(self, f"{location}_x_motor"),
                        getattr(self, f"{location}_y_motor"),
                        getattr(self, f"{location}_z_motor"),
                    ]
                )
                setattr(self, location, posns)
                posns = np.array(
                    [
                        getattr(self, f"{location}_x_motor_enc"),
                        getattr(self, f"{location}_y_motor_enc"),
                        getattr(self, f"{location}_z_motor_enc"),
                    ]
                )
        ...
        if successes["F0"] + successes["F1"] + successes["F2"] == 2:
            ...
            yield from bps.mv(
                self.x, locations[fail_loc][0], self.y, locations[fail_loc][1]
            )
            success = yield from self.find_center(fail_loc, center_high=True)
            if success:
                posns = np.array(
                    [
                        getattr(self, f"{fail_loc}_x_motor"),
                        getattr(self, f"{fail_loc}_y_motor"),
                        getattr(self, f"{fail_loc}_z_motor"),
                    ]
                )
                setattr(self, fail_loc, posns)
                posns = np.array(
                    [
                        getattr(self, f"{fail_loc}_x_motor_enc"),
                        getattr(self, f"{fail_loc}_y_motor_enc"),
                        getattr(self, f"{fail_loc}_z_motor_enc"),
                    ]
                )
                ...
                print(
                    f"Fiducial at {fail_loc} failed to be found twice, set using manual_set_fiducial routine."
                )
        ...
        if all(value == True for value in successes.values()):
            ...
            if not (25300 < lx < 25500):
                print(
                    f"Horizontal distance between vectors {lx} appears to be off (expected ~25400), please check."
                )
            if not (25300 < ly < 25500):
                print(
                    f"Vertical distance between vectors {ly} appears to be off (expected ~25400), please check."
                )
        else:
    ...
    def calculate_fit(self):
        if self.F0 and self.F1 and self.F2:
            ...
            print(
                f"Angle between vectors: {theta:6.4f}radians ({theta*180/np.pi:6.3f}deg)"
            )
            return (
                np.linalg.norm(ab),
                np.linalg.norm(ac),
                np.arccos(np.clip(cos_theta, -1, 1)),
            )
    ...
    def find_center(self, location, center_high=False):
        ...
        success, size = yield from autofocus(
            self.hi_camera, "stats4_sigma_x", self.z, -100, 100, 15
        )
        yield from self.find_camera_center(location, "hi_camera", center=center_high)
        if success and (size > 20):
            ...
            print(
                f"Detected hole at {location} location appears too small (sigma_x<20), please confirm using manual_set_fiducial routine."
            )
        else:
            print(
                f"Failed to find fiducial at {location} location, please find using manual_set_fiducial routine."
            )
    ...
    def find_camera_center(self, location, camera_st, center=True):
        ...
        if camera == cam_7:
            ...
            roi4_focus_size = (
                100  # Size to cover large fiducial, not the small neighbors
            )
            MagCal = BL_calibration.LoMagCal.get()
        elif camera == cam_8:
            zoom_roi = camera.roi1
            roi4_focus_size = (
                200  # Size to cover large fiducial, not the small neighbors
            )
        ...
        zoom_roi_center_x = zoom_roi.min_xyz.min_x.get() + np.ceil(
            zoom_roi.size.x.get() / 2
        )
        zoom_roi_center_y = zoom_roi.min_xyz.min_y.get() + np.ceil(
            zoom_roi.size.y.get() / 2
        )

        yield from bps.abs_set(
            camera.roi4.min_xyz.min_x, max_x - roi_size / 2, wait=True
        )
        yield from bps.abs_set(
            camera.roi4.min_xyz.min_y, max_y - roi_size / 2, wait=True
        )
        ...
        yield from bps.abs_set(
            camera.roi4.min_xyz.min_x, zoom_roi_center_x - roi_size / 2, wait=True
        )
        yield from bps.abs_set(
            camera.roi4.min_xyz.min_y, zoom_roi_center_y - roi_size / 2, wait=True
        )
    ...
    def drive_to_location(self, location_name):
        ...
        yield from bps.mv(
            self.x, motor_loc[0], self.y, motor_loc[1], self.z, motor_loc[2]
        )
    ...
    def configure_detector(self, location, triggers):
        ...
        if not self.filepath:
            print(
                f"Must set filepath attribute for this chip scanner object before taking data, to determine location where the file will be saved."
            )
        ...
        if not eiger_single.cam.file_path_exists.get():
            print(
                f"Filepath {self.filepath} does not appear to be a valid directory recognized by the Eiger detector."
            )
            raise Exception("Filepath not found")
    ...
    def ppmac_linear_scan(
        ...
        num_steps,
        start_offset=0.01,
        location_offset_x=0.0,
        location_offset_y=0.0,
    ):
        direction = np.sign(step_vector[0])
        zebra.pc.direction.put((1 - direction) / 2)
        zebra.pc.gate.width.put(abs(step_vector[0] / 50.0))
        ...
        lse = (
            location_start_enc
            - step_vector_enc * location_offset_x
            + np.array([0, 1, 0]) * location_offset_y * 10000
        )
        input_array = [
            [lse[0] + i * step_vector_enc[0], lse[1] + i * step_vector_enc[1]]
    ...
    def ppmac_single_line_scan(
        ...
        if zebra.pc.gate.sel.get():
            print(
                "Zebra appears to be configured for gonio1, run configure_zebra_for_chip_scanner() and retry."
            )
            return False
        a0 = self.name_to_fiducial_distances("A1aa")
        ax = self.name_to_fiducial_distances("A1ab")
        ay = self.name_to_fiducial_distances("A1ba")
        x_step = self.fiducial_distances_to_location(
            *ax
        ) - self.fiducial_distances_to_location(*a0)
        y_step = self.fiducial_distances_to_location(
            *ay
        ) - self.fiducial_distances_to_location(*a0)
        x_step_enc = self.fiducial_distances_to_enc_location(
            *ax
        ) - self.fiducial_distances_to_enc_location(*a0)
        y_step_enc = self.fiducial_distances_to_enc_location(
            *ay
        ) - self.fiducial_distances_to_enc_location(*a0)
        ...
        loc = np.array(
            [
                self.x.get().user_readback,
                self.y.get().user_readback,
                self.z.get().user_readback,
            ]
        )
        enc_loc = np.array(
            [
                self.x.get().encoder_readback,
                self.y.get().encoder_readback,
                self.z.get().encoder_readback,
            ]
        )
        govStateSet("CD", configStr="Chip_Scanner")
        status = yield from self.ppmac_linear_scan(
            loc,
            enc_loc,
            x_step,
            x_step_enc,
            wait_time,
            20,
            start_offset=zebra_offset + location_offset_x,
            location_offset_x=location_offset_x,
            location_offset_y=location_offset_y,
        )
        between_time = wait_time * 20 + 400
        status.wait(between_time / 1000.0 + 20)
        ...
        print(
            f"Data location = {eiger_single.cam.file_path.get()}{eiger_single.cam.fw_name_pattern.get()}"
        )

    def ppmac_neighbourhood_scan(
        self,
        neighbourhood,
        wait_time,
        zebra_offset=0.01,
        ...
        if not pattern.match(neighbourhood):
            print(
                f"Neighbourhood scan requires input of form ex. A1, got {neighbourhood}."
            )
            return False
        if zebra.pc.gate.sel.get():
            print(
        ...
        for i in range(10):
            ...
            status = yield from self.ppmac_linear_scan(
                ...
                location_offset_x=location_offset_x,
                location_offset_y=location_offset_y,
            )  # 20 to push the chip off the sample at the end
            status.wait(between_time / 1000.0 + 20)
            shutter_bcu.close.put(1)
    ...
    def linear_scan_with_triggering(self, location_start, location_end, wait_time):
        ...
        if Ny != ord(location_end[0]) - 65 or Nx != int(location_end[1]) - 1:
            print(
                "linear_scan is not yet configured to work with houses in different neighbourhoods"
            )
            return ()
        ...
        yield from bps.mvr(
            self.x, -direction * 5
        )  # Move away so that the first trigger hits on the first point
        ...
        for _ in range(num_steps):
            ...
            yield from bps.mvr(
                self.x, motor_d[0], self.y, motor_d[1], self.z, motor_d[2]
            )
    ...
    def check_camera_settings(self, camera):
        if camera.cam.acquire_time.get() > 0.1:
            print(
                f"{camera} acquireTime > 0.1, which could affect processing. Please reduce this to continue."
            )
            return False
        if camera.cam.image_mode.get() != 2:
            print(
                f"{camera} not in continuous acquisition mode, please set this and retry."
            )
            return False
